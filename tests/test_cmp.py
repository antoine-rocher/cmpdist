...
@pytest.mark.parametrize("nu", [0.6, 0.8, 1.0, 1.5, 2.5, 3.9])
...
@pytest.mark.parametrize("nu", [0.6, 1.0, 1.9, 3.9])
...
@pytest.mark.parametrize("nu", [0.54, 0.49, 0.3, 0.0, 4.01, 5.0, -1.0])
def test_nu_outside_valid_range_raises(d, nu):
    """Default policy rejects nu outside the closed interval [0.55, 4]."""
    with pytest.raises(ValueError, match=r"0\.55 <= nu <= 4"):
        d.sample(np.full(10, 1.0), nu)


@pytest.mark.parametrize("nu", [0.55, 0.6, 0.75, 1.0, 1.5, 2.0, 3.0, 4.0])
def test_nu_inside_valid_range_accepted(d, nu):
    """The closed interval endpoints 0.55 and 4.0 are accepted."""
...
def test_on_invalid_nu_warn_mode():
    ...
    with pytest.warns(RuntimeWarning, match=r"0\.55 <= nu <= 4"):
...
def test_custom_nu_range():
    ...
    with pytest.raises(ValueError, match=r"0\.1 <= nu <= 5"):
        wide.sample(np.full(10, 1.0), 0.09)
...
@pytest.mark.parametrize("nu", [0.6, 0.8, 1.0, 1.5, 2.0, 3.0, 3.9])
...
def test_nu_lower_endpoint_is_the_documented_exception():
    """nu = 0.55 is the excluded lower bound and is the one point exceeding 5%.

    Documented as ~5.2%, confined to lam near the branch crossover. If this
    tightens or worsens, the docs and README table need updating with it.
    """
    d = CMP(nu_range=None)
    lam = np.geomspace(1e-3, 1e3, 200)
    bias = np.array([
        (np.arange(d.max_count(l) + 1) * d.pmf(np.arange(d.max_count(l) + 1), l, 0.55)).sum() / l - 1
        for l in lam
    ])
    assert 0.05 < np.abs(bias).max() < 0.06
